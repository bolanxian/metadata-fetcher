...
const searchAsRedirect = async (url, resolved) => {
  const target = await redirect(resolved.url)
  if (target == null) { return $html('default', resolved.id) }
  return $redirect(new URL(`./.search?.=${encodeURIComponent(target)}`, url).href)
}
$.search = ({ url }) => {
  ...
  if (startsWith(id, '@redirect!')) {
    return searchAsRedirect(url, resolved)
  }
  return $html('default', input)
}
