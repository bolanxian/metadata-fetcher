...
import {
  ...
  cache, discoverMap, discoverHttpMap,
  redirect, xresolve, resolve, xparse,
...
<script defer src="./.version-check"></script>
...
defineStaticFile('version-check', types.js, `\
try {
  /**
  ...
  document.addEventListener('DOMContentLoaded', function (e) {
    var icon = 'at' in Array.prototype 
      ? '<i class="ivu-icon ivu-icon-ios-close-circle"></i>' 
      : '<img style="height:16px" src="./.favicon">'
    var msg = '当前浏览器版本' + ('with' in Array.prototype ? '较' : '过') + '低！'
    var div = document.createElement('div')
    div.innerHTML = '\
<div class="ivu-alert ivu-alert-error ivu-alert-with-icon">\
<span class="ivu-alert-icon">' + icon + '</span>\
<span class="ivu-alert-message">' + msg + '</span>\
</div>\
'
...
$.suggest = ({ url }) => {
  const input = trim(url.searchParams.get('.') ?? '')
  if (!input) {
    const data = stringify([input, [...cache.keys()]])
  ...
  return new Response(stringify([input, ids]), {
