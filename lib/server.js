...
import {
  name, ready, $string, $array, getOwn, match, split, encodeText as encode, join,
  config, readConfig, writeConfig, resolve, xparse, render, redirect,
  S, renderToHtml, createBatchParams, handleRequestBbdown
...
$.opensearch = (ctx) => {
  const url = new URL(base, ctx.url).href
  return new Response(`\
<OpenSearchDescription xmlns="http://a9.com/-/spec/opensearch/1.1/">
  ...
  <Url type="${types.html}" method="get" template="${url}.search?.={searchTerms}"/>
</OpenSearchDescription>
`, {
    headers: {
      server,
      'content-type': types.osdx,
      'cache-control': 'public, max-age=86400'
    }
  })
}
$.search = ({ url }) => {
  ...
  let id = resolve(input)?.id ?? input
  if (id[0] === '.') { id = slice(id, 1) }
...
const REG_ID = /(?<![a-z])[a-z]{2}\d+|youtu\.?be[!:./?&=-\w]+/g
const matcher = (getIter) => async (ctx) => {
  ...
  const location = new URL(`./.batch?${createBatchParams('.id', set)}`, ctx.url).href
  return new Response(null, {
    status: 302,
    headers: { server, location }
  })
...
$.redirect = async ({ url }) => {
  ...
  let id = target != null ? resolve(target)?.id ?? target : input
  if (id[0] === '.') { id = slice(id, 1) }
  const location = new URL(`./${encodeURIComponent(id)}`, url).href
  return new Response(null, {
    status: 302,
